...
=begin 
      # get_locator above does not follow the bitcoin protocol
      # this one does but needs optimizing
      def get_locator(hash)
        ...
        while blk
          have << blk.hash
          step.times do
            blk = blk.get_prev_block
            break if not blk
          end
          step = step * 2 if have.size > 10
        end
        have << Bitcoin::network[:genesis_hash]
      end
=end 
